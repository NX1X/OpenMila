...
- **A Windows installer.** `OpenMila-1.9.5-beta.2+port.3-win64-setup.exe`
...
- `OpenMila-1.9.5-beta.2+port.3-x86_64.AppImage` - `chmod +x` it and run it. Run
  it with `--install` to add a desktop entry, `--uninstall` to remove one.
- `openmila_1.9.5-beta.2+port.3_amd64.deb` - `sudo apt install ./openmila_*.deb`.
