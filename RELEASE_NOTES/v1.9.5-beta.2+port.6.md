...
**This build makes the Windows window usable.** The first hands-on report
was "a third of the page and black boxes", and it was accurate: the Home page
sat in a narrow middle column with the wide pane blank, and buttons were
squeezed until their labels wrapped letter by letter ("Setting / s"). The
cause was how the toolkit's split view behaves on Windows, where a pane is
fixed at its minimum width. Home now takes the wide pane, the recordings
list has a real width beside its detail, buttons take their natural size,
and the version sits on its own line. Windows CI now starts the packaged app
and photographs both layouts on every build, which is how this was seen and
how it was confirmed fixed.

Also in this build:

- The log speaks in the port's own name. Every line used to carry Mila's
  subsystem names (`io.island...`); they are remapped to
  `io.github.nx1x.openmila` without touching upstream's files.
- Local AI setup logs every stage, and the provider Test logs its result, so
  a report from another machine says where a setup stopped.
- A weekly check that the vendor still publishes the digests pinned for the
  local AI runtime. Ollama ships no signatures, so that pin is the defence.

**Since the previous build: local AI, set up from inside the app.** Settings > AI
...
- **A Windows installer.** `OpenMila-1.9.5-beta.2+port.6-win64-setup.exe`
...
- `OpenMila-1.9.5-beta.2+port.6-x86_64.AppImage` - `chmod +x` it and run it. Run
  it with `--install` to add a desktop entry, `--uninstall` to remove one.
- `openmila_1.9.5-beta.2+port.6_amd64.deb` - `sudo apt install ./openmila_*.deb`.
...
**Windows 11**: run `OpenMila-1.9.5-beta.2+port.6-win64-setup.exe`, which
installs for your user only and needs no administrator. If you would rather
install nothing, unzip `OpenMila-1.9.5-beta.2+port.6-win64.zip` anywhere and
