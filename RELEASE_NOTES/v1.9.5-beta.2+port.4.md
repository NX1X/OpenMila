...
- **A Windows installer.** `OpenMila-1.9.5-beta.2+port.4-win64-setup.exe`
...
- `OpenMila-1.9.5-beta.2+port.4-x86_64.AppImage` - `chmod +x` it and run it. Run
  it with `--install` to add a desktop entry, `--uninstall` to remove one.
- `openmila_1.9.5-beta.2+port.4_amd64.deb` - `sudo apt install ./openmila_*.deb`.
...
**Windows 11**: run `OpenMila-1.9.5-beta.2+port.4-win64-setup.exe`, which
installs for your user only and needs no administrator. If you would rather
install nothing, unzip `OpenMila-1.9.5-beta.2+port.4-win64.zip` anywhere and
