...
;
; The cache itself is cleared from [Code] (ClearCacheExceptLocalAI): this
; section cannot skip one entry by name, and "Excludes" belongs to [Files].
...
// The regenerable cache goes on a plain uninstall, except local-ai: that is the
// model server and the language models the user chose to download from inside
// the app, several gigabytes, and a plain uninstall must not throw them away
// unasked. The purge removes the whole {localappdata}\OpenMila tree, so it
// takes them.
procedure ClearCacheExceptLocalAI;
var
  Cache: String;
  Entry: TFindRec;
begin
  Cache := ExpandConstant('{localappdata}\{#MyAppName}\cache');
  if not DirExists(Cache) then
    exit;
  if FindFirst(Cache + '\*', Entry) then
  begin
    try
      repeat
        if (Entry.Name <> '.') and (Entry.Name <> '..') and (CompareText(Entry.Name, 'local-ai') <> 0) then
        begin
          if (Entry.Attributes and FILE_ATTRIBUTE_DIRECTORY) <> 0 then
            DelTree(Cache + '\' + Entry.Name, True, True, True)
          else
            DeleteFile(Cache + '\' + Entry.Name);
        end;
  ...
  // Empty now unless local-ai stayed; RemoveDir refuses a non-empty directory.
  RemoveDir(Cache);
end;
