...
stage_port() {
  log "port modules: recording session, time stretching, self-update, Linux platform services"
  retry swift test "${FLAGS[@]}" --filter '^(RecordingTests|StretchTests|UpdaterTests|LinuxPlatformTests|LocalAITests)\.'
}
